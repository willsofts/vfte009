...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte009" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
  ...
  setup() {
    ...
    const dataCategory = {
      ...
      tkvisible: [{id: "0", text: "Visible"},{id: "1", text: "Invisible"}],
      tksystemtype: [],
      tkdomaintype: [],
      tkdomainappstype: [],
    };
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vfte009",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte009", (success) => {
            if (success) {
  ...
  methods: {
    ...
    loadDataCategories(loading,callback) {
      ...
      if(!loading) return;
      let jsondata = {names: ["tkactive", "tkvisible", "tksystemtype", "tkdomaintype", "tkdomainappstype"]};
    ...
    resetDataCategories(lang) {
      ...
      tk_category = this.dataChunk["tkvisible"];
      if(tk_category) {
        tkvisible = tk_category.map((item) => { return { id: item.typeid, text: "TH"==lang?item.nameth:item.nameen } });
      }
      tk_category = this.dataChunk["tksystemtype"];
      if(tk_category) {
        tksystemtype = tk_category.map((item) => { return { id: item.typeid, text: "TH"==lang?item.nameth:item.nameen } });
      }
      tk_category = this.dataChunk["tkdomaintype"];
      if(tk_category) {
        tkdomaintype = tk_category.map((item) => { return { id: item.typeid, text: "TH"==lang?item.nameth:item.nameen } });
      }
      tk_category = this.dataChunk["tkdomainappstype"];
      if(tk_category) {
        tkdomainappstype = tk_category.map((item) => { return { id: item.typeid, text: "TH"==lang?item.nameth:item.nameen } });
      }
      if(tkactive) this.dataCategory.tkactive = tkactive;
      if(tkvisible) this.dataCategory.tkvisible = tkvisible;
      if(tksystemtype) this.dataCategory.tksystemtype = tksystemtype;
      if(tkdomaintype) this.dataCategory.tkdomaintype = tkdomaintype;
      if(tkdomainappstype) this.dataCategory.tkdomainappstype = tkdomainappstype;
    },
    dataSelected(item,action) {
      //listen action from search form
      console.log("App: dataSelected",item,"action",action);
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({domainid: item.domainid});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({domainname: item.domainname, domainid: item.domainid});
      }
    },
    dataInsert(filters) {
      //listen action from search form
